...
def attack_set(files, mean, std, rng: random.Random,
    ...
    for path in files:
        ...
        for t, vec in resample(hurt, period, max_hold):
            ...
            labels.append(bool(span) and span["start"] <= t <= span["stop"])
            previous = t
        if span:
