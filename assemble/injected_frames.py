...
def write_and_pass_frames(injected, dest, shard):
    """Write the frames of each log `injected` yields to `dest`, and yield the log as it
    came.

    The files are `dest/test-NNNNN.parquet`, each holding about `shard` frames.
    """
    ...
    for path, frames, hurt, span in injected:
        batches.append(pa.RecordBatch.from_pydict(_columns(path, frames, hurt),
                                                  schema=SCHEMA))
        if sum(b.num_rows for b in batches) >= shard:
            flush()
