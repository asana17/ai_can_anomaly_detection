...
def hold_out(logs: Iterable[str], frac: float, weight=None, blocks: int = 1,
             gap: int = 0):
    """Take `blocks` stretches out of the logs for calibration, and return both parts.
    ...
    logs on each side are dropped from both parts, because a log next to a held out
