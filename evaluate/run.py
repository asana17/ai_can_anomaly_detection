...
from assemble.datasets import save, scaled_rows
...
FILES = 1200            # logs to sample, spread evenly over the recording
...
def main(pattern, out_dir):
    ...
    save(data, out_dir)
    scale = data["scale"]
    print(f"arrays in {time.time() - clock:.0f}s, train {data['train'].shape}",
          flush=True)

    clock = time.time()
    got = attack_set(test, scale, random.Random(0),
                     source_logs=train[::max(len(train) // DONORS, 1)][:DONORS])
    for name in ("rows", "raw", "t", "seg", "label"):
    ...
    print(f"attack set in {time.time() - clock:.0f}s, {len(got['attacks'])} attacks",
          flush=True)
