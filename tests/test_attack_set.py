...
def _stats():
    return np.zeros(SIGNALS, dtype=np.float32), np.ones(SIGNALS, dtype=np.float32)
...
def test_a_log_too_short_to_attack_still_contributes_rows(tmp_path):
    d = attack_set([_write_log(tmp_path / "a.csv", seconds=5)], *_stats(), random.Random(0))
    assert len(d["rows"]) > 0
    assert d["attacks"] == []
...
def test_one_seed_gives_one_set(tmp_path):
    log = _write_log(tmp_path / "a.csv")
    a = attack_set([log], *_stats(), random.Random(3))
