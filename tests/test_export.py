...
from board.export import load, write
...
def test_the_int8_file_runs_on_the_same_rows(tmp_path):
    rows = _rows()
    write(_model(), rows, str(tmp_path / "out"), "ae", batch=128)
...
def test_only_the_float_and_int8_files_are_kept(tmp_path):
    write(_model(), _rows(), str(tmp_path / "out"), "ae", batch=128)
...
def test_write_never_overwrites_an_export(tmp_path):
    ...
    with pytest.raises(FileExistsError):
        write(_model(), _rows(), str(dest), "ae", batch=128)
