...
def stopped_shaft_hits(board_rule):
    return board_rule("stopped_shaft", ["float"] * 2)


def _rows(rng):
    """Rows with the wheels mostly stopped, as float32 like the board's.

    NaN fills 1% of cells.
    """
    columns = {"wheel_speed": np.where(rng.random(ROWS) < 0.7, 0.0,
                                       rng.uniform(0.0, 5.0, ROWS)).astype(np.float32),
               "output_shaft_speed": (rng.uniform(0.0, 2.0, ROWS) * MAX_SHAFT)
               .astype(np.float32)}
...
def test_the_c_port_matches_the_python_rule(stopped_shaft_hits):
    ...
    columns = [raw[:, SIGNALS.index(name)] for name in
               ("wheel_speed", "output_shaft_speed")]
